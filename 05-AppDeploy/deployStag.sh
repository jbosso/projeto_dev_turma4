...
export SQLstage=$($uri 'aws --region sa-east-1 ec2 describe-instances --filters Name=tag:Name,Values=mysql-stag --query "Reservations[*].Instances[*].[PrivateIpAddress]" --output text | grep 172')
